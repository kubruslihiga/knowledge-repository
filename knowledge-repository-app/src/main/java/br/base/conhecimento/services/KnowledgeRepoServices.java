...
public class KnowledgeRepoServices {
	...
	public Knowledge insertKnowledge(Knowledge knowledge) throws Exception {
		...
		try {
			...
			String knowledgeURIBase = BASE + knowledge.getName().replaceAll(" ", "_");
	...
	public void selectKnowledge() throws Exception {
		...
		try {
			dataset = TDBFactory.createDataset("/home/mauricio/tdb");
			getOntModel(dataset, ReadWrite.READ);
	...
	public void insertIndividual() {
		...
		try {
			dataset = TDBFactory.createDataset("/home/mauricio/tdb");
			OntModel ontModel = getOntModel(dataset, ReadWrite.WRITE);
			Resource ontologyInstance = ontModel.getResource("http://base.example/Forma_cabelo");
			Individual individual = ontModel.createIndividual("http://base.example/Forma_cabelo/Liso",
					ontologyInstance);
			individual.addLiteral(ontModel.createProperty("http://base.example/Forma_cabelo/nome"), "Preto");
			ontModel.commit();
			dataset.commit();
			ontModel.write(System.out);
		} finally {
